...
impl DemoGame {
        ...
        self.bump_allocator = {
            {
                ...
                for asteroid_entity in hit_asteroids.drain(..) {
                    ...
                    ctx.play_sound(self.sounds.asteroid_explode.clone());
    ...
    pub fn update_player_collisions(&mut self, ctx: &mut RetroBlitContext) {
        let bump_allocator = std::mem::take(&mut self.bump_allocator);
        self.bump_allocator = {
            if let Some(player_entity) = self.player_entity {
                ...
                if let Ok((player_position, player_rotation, player_handle)) = prh {
                    ...
                    if !hit_asteroids.is_empty() {
                        ctx.play_sound(self.sounds.player_explode.clone());
